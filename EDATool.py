...
def func():
    ...
    try:
        if(len(st.session_state.keys())!=5):
            ...
            st.write(set(['Existing Disease','Symptoms','Age','Gender','Zipcode']).difference(set(st.session_state.keys())))
        else:
        ...
        ##        test_df.loc[0,i]=1
            ...
            typ = st.radio("", ('LIKELIHOOD', 'SPECIALITY'))
            if(typ=='LIKELIHOOD'):
                st.write("Diseases",prediction_1_df_out)
            if(typ=='SPECIALITY'):
                st.write("Speciality",prediction_1_df_out)
    except:
...
options = st.sidebar.radio('Please Enter Below Details:', ['Existing Disease', 'Symptoms', 'Age','Gender','Zipcode','Submit'])
